...
void lcd_init(void)
{
	...
	lcd_send_command(LCD_DISPLAY_ON_UNDERLINE_OFF_CURSOR_OFF);
}
...
void lcd_set_position(unsigned char line ,unsigned char pos)
{
	if(line == 0){
		...
		if(pos <16 && pos>=0){
			lcd_send_command(LCD_BEGIN_AT_FIRST_RAW+ 0x10 +pos);
		}
	}
	else{
		if(pos <16 && pos>=0){
			lcd_send_command(LCD_BEGIN_AT_SECOND_RAW +0x10 +pos);
		}
	}
